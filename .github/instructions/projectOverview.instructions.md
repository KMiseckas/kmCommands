...
- `src/CommandConfig.cs`: public `CommandConfig` sealed class; internal constructor; `HistoryCapacity` (int, default `DefaultHistoryCapacity`) and `DevMode` (bool, default false); `FromJson(string)` and `FromFile(string)` static factories returning `ConfigResult`; hand-rolled parser via `JsonConfigParser`; case-insensitive key matching; unknown keys → warnings; type mismatch → `Fail`.
